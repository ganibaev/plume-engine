...
void main()
{
	...
	vec3 dirDiffuse = diffuseDir * inColor;
	vec3 dirSpecular = specularDir * inColor;
	...
	for (int i = 0; i < NUM_LIGHTS; ++i)
	{
		...
		resColor += (diffusePointLight * inColor + specularPointLight * inColor);
	}
	
	resColor += (ambientLight * inColor);

	outFragColor = vec4(resColor, 1.0f);
}